...
program
    ...
    .hook('preAction', async (thisCommand) => {
        ...
        // Set network
        Dexterity.config.network = options.network === 'mainnet' ? STACKS_MAINNET : STACKS_TESTNET;
        ...
        if (options.debug) {
            debugUtils.setDebugMode({
                ...
                verbosity: 2,
                callback: (info) => {
                    console.log(chalk.gray(`Debug [${info.phase}]:`, JSON.stringify(info.details, null, 2)));
                }
            });
        }
    });
    ...
    .action(async (cmd, options) => {
        ...
        if (options.args.length === 0) {
            ...
            Object.entries(displayConfig).forEach(([key, setting]) => {
                const valueColor = setting.source === 'user' ? chalk.green : chalk.yellow;
                const sourceLabel = setting.source === 'user' ? chalk.blue('(custom)') : chalk.gray('(default)');
                console.log(`${chalk.cyan(key.padEnd(16))}: ${valueColor(setting.value)} ${sourceLabel}`);
            });

            console.log('\nConfig file location:', chalk.gray(CONFIG_FILE));
            if (Object.keys(userConfig).length === 0) {
                console.log(chalk.yellow('\nNo custom configuration set. Using default values.'));
                console.log(`To set a value, use: ${chalk.cyan('dexterity config set <key> <value>')}`);
            }
        }
        else if (options.set) {
            ...
            // Validate the key
            const validKeys = ['network', 'mode', 'maxHops', 'defaultSlippage', 'proxy', 'apiKey'];
            ...
            try {
                ...
                switch (key) {
                    ...
                    case 'defaultSlippage':
                        ...
                        if (isNaN(slippage) || slippage < 0 || slippage > 100) {
                            throw new Error('defaultSlippage must be a number between 0 and 100');
                        }
                ...
                saveConfig(config);
                console.log(chalk.green(`Successfully set ${chalk.cyan(key)} = ${chalk.yellow(parsedValue)}`));
            ...
            const validKeys = ['network', 'mode', 'maxHops', 'defaultSlippage', 'minimumLiquidity', 'proxy', 'apiKey'];
    ...
    .action(async (options) => {
        try {
            ...
            if (options.pool) {
                ...
                console.log('\nReserves:');
                console.log(`${poolData.liquidity[0].symbol}: ${chalk.yellow(reserve0.toLocaleString())}`);
                console.log(`${poolData.liquidity[1].symbol}: ${chalk.yellow(reserve1.toLocaleString())}`);
            }
                ...
                if (pools.size > 0) {
                    ...
                    for (const pool of pools.values()) {
                        const poolData = pool.getPool();
                        console.log(`- ${chalk.blue(poolData.name)} (${poolData.contractId})`);
                    }
                }
            }
                ...
                const route = await Dexterity.router.findBestRoute(tokenIn, tokenOut, 1000000);
                if (route.isErr()) {
                ...
                bestRoute.path.forEach((token, i) => {
                    if (i < bestRoute.path.length - 1) {
                        console.log(`${chalk.cyan(token.symbol)} → ${chalk.cyan(bestRoute.path[i + 1].symbol)}`);
                    }
                });
                ...
                console.log(`Density:   ${chalk.cyan((stats.edgeCount / (stats.nodeCount * (stats.nodeCount - 1))).toFixed(3))}`);

                if (debugUtils.config.enabled) {
                    const debugStats = debugUtils.getStats();
                    console.log('\nDebug Statistics:');
                    console.log(`Paths Explored:    ${chalk.yellow(debugStats?.pathsExplored)}`);
                    console.log(`Routes Evaluated:  ${chalk.yellow(debugStats?.routesEvaluated)}`);
                    console.log(`Quotes Requested:  ${chalk.yellow(debugStats?.quotesRequested)}`);
                }
            }
        } catch (error) {
            console.error(chalk.red('Error:'), error instanceof Error ? error.message : error);
...
program
    ...
    .action(async (tokenIn: ContractId, tokenOut: ContractId, amount: string) => {
        try {
            ...
            const quote = await Dexterity.getQuote(tokenIn, tokenOut, parseInt(amount));
            spinner.stop();

            console.log('\nQuote Details:');
            console.log('─────────────────────────────────');
            console.log(`Amount In:        ${chalk.cyan(quote.amountIn.toLocaleString())} ${tokenIn}`);
            console.log(`Amount Out:       ${chalk.green(quote.amountOut.toLocaleString())} ${tokenOut}`);
            console.log(`Expected Price:   ${chalk.yellow(quote.expectedPrice.toFixed(6))}`);
            ...
            if (quote.route.hops.length > 1) {
                ...
                quote.route.hops.forEach((hop, i) => {
                    console.log(`${i + 1}. ${chalk.blue(hop.tokenIn.symbol)} → ${chalk.blue(hop.tokenOut.symbol)}`);
                });
            }
        } catch (error) {
            console.error(chalk.red('Error:'), error instanceof Error ? error.message : error);
    ...
    .action(async () => {
        try {
            ...
            pools.forEach((pool) => {
                ...
                console.log(`Tokens:   ${pool.liquidity[0].symbol}-${pool.liquidity[1].symbol}`);
